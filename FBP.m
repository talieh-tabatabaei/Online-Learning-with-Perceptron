function [SV,G,Y,Index,hat_y_t] = FBP(x_t,y_t,index,SV,G,Y,Index,ker,p1,p2,C,B)
%Online budget perceptron algorithm with fixed size of B (fixed BP)
% INPUT:
...
%% Making Update to the SV set
   ...
   if (l_t > 0)
       if N>B
           [SV,Y,G,Index] = remove(SV,Y,G,Index); %first remove one SV usifn remove function
           N=N-1;
...
function [SV2,Y2,G2,Index] = remove(SV,Y,G,Index)
...
for i=1:N
    ...
    l_t(i,1)= max(0,1-y_t*f_t); %Hing loss
end
[minimum j]=min(l_t);
